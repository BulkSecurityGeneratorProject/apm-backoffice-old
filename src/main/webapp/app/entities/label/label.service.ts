...
export class LabelService {
    ...
    create(label: ILabel): Observable<EntityResponseType> {
        const copy = this.convertDateFromClient(label);
        return this.http
    ...
    protected convertDateFromClient(label: ILabel): ILabel {
        const copy: ILabel = Object.assign({}, label, {
            creationDate: label.creationDate != null && label.creationDate.isValid() ? label.creationDate.format(DATE_FORMAT) : null,
            updateDate: label.updateDate != null && label.updateDate.isValid() ? label.updateDate.format(DATE_FORMAT) : null
        });
        return copy;
    }
    ...
    protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        if (res.body) {
            res.body.forEach((label: ILabel) => {
                label.creationDate = label.creationDate != null ? moment(label.creationDate) : null;
                label.updateDate = label.updateDate != null ? moment(label.updateDate) : null;
            });
        }
        return res;
    }
